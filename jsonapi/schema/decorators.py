...
def validates(field, when="post-decode", context="always"):
    """
    ...
    :arg str context:
        Must be either *always*, *never*, *oncreation* or *onupdate*.
    """
    assert when in ("pre-decode", "post-decode")
    assert context in ("never", "always", "oncreation", "onupdate")

    def decorator(f):
        f.japi_validator = {"field": field, "when": when, "context": context}
        return f
    return decorator
